...
// Task 3: Full SaaS-quality responsive UI
// Desktop: 3-column layout (nav sidebar | main content | right panel)
// Mobile: bottom nav + single column
// Tablet: 2-column with collapsible right panel
...
export default function CirclePage() {
  ...
  const { verse, chapter, audio, tafsir, loading: ayahLoading, tafsirLoading, error: ayahError, lens, verseKey, dayNumber, fetchTafsirForToday } = useAyah()
  ...
  const reflectedCount = members.filter((m) => m.has_reflected_today).length
  ...
  useEffect(() => {
    getClientAccessToken().then(async (token) => {
      ...
      if (streak) setStreakCount(streak.current_streak)
    }).catch(() => null)
  }, [])
  ...
  useEffect(() => {
    ...
    getClientAccessToken().then(async (token) => {
      if (!token) return
      const items = await getCollections(token)
  ...
  async function handleBookmark() {
    ...
    if (!token) { setBookmarkLoading(false); return }
    const saved = await bookmarkVerse(token, verseKey)
    setBookmarkLoading(false)
    setActionMessage(saved ? "Ayah bookmarked ✓" : "Bookmarking failed")
  ...
  async function handleCollectionSave() {
    ...
    const token = await getClientAccessToken()
    if (!token) { setCollectionLoading(false); return }
    let id = selectedCollectionId
    if (!id && newCollectionName.trim()) {
      const created = await createCollection(token, newCollectionName.trim())
      id = created?.id ?? null
    }
    if (!id) { setCollectionLoading(false); setActionMessage("Choose or create a collection first."); return }
    const added = await addToCollection(token, id, verseKey)
    setCollectionLoading(false); setCollectionSheetOpen(false)
    setSelectedCollectionId(null); setNewCollectionName("")
    setActionMessage(added ? "Added to collection ✓" : "Couldn't add ayah")
    setTimeout(() => setActionMessage(null), 2500)
  }
  ...
  async function handleSaveNote() {
    setNoteLoading(true)
    const token = await getClientAccessToken()
    if (!token) { setNoteLoading(false); return }
    const saved = await createNote(token, verseKey, noteBody.trim())
    setNoteLoading(false)
    if (!saved) { setActionMessage("Couldn't save note"); return }
    setNoteBody(""); setNoteSheetOpen(false)
  ...
  async function handleSubmitReflection() {
    ...
    if (body.length < 6) { setActionMessage("Reflection must be at least 6 characters."); return }
    const created = await postReflection(body, selectedLens)
    if (created) { setComposerBody(""); setSelectedLens(lens) }
  }
    ...
    <div className="min-h-screen bg-[var(--ink)] text-[var(--text)]">
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 h-full w-60 flex-col border-r border-white/6 bg-[rgba(15,14,12,0.8)] backdrop-blur-xl z-30 p-4">
        <div className="mb-8 px-2">
          ...
          <p className="text-xs text-[var(--muted)] mt-0.5">{room?.name ?? "Your Circle"}</p>
        </div>
        <SidebarNav />
        <div className="mt-auto">
          <div className="rounded-2xl border border-[var(--gold-border)] bg-[var(--gold-dim)] px-3 py-2">
            <p className="text-xs text-[var(--muted)]">Streak</p>
            <p className="text-xl font-bold text-[var(--gold)]">{streakCount} days</p>
          </div>
        </div>
      </aside>

      {/* ── Main layout ── */}
      <div className="lg:ml-60 xl:mr-80">
        {/* Mobile header */}
        <header className="lg:hidden sticky top-0 z-30 px-4 pt-3 pb-2 bg-[rgba(15,14,12,0.85)] backdrop-blur-xl border-b border-white/6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setRightPanelOpen(!rightPanelOpen)} className="rounded-xl border border-white/10 bg-white/4 px-3 py-1.5 text-xs text-[var(--muted)]">
                Members
              </button>
              <div className="rounded-full border border-[var(--gold-border)] bg-[var(--gold-dim)] px-2.5 py-1 text-xs text-[var(--gold)]">{streakCount}d</div>
            </div>
          </div>
        </header>

        {/* Desktop section title bar */}
        <div className="hidden lg:flex items-center justify-between px-8 pt-6 pb-4">
          ...
          <button type="button" onClick={() => setInviteOpen(true)} className="rounded-xl border border-white/10 bg-white/4 px-4 py-2 text-sm hover:border-[var(--gold-border)] hover:bg-[var(--gold-dim)] hover:text-[var(--gold)] transition-all">
            + Invite Members
          </button>
        </div>
        ...
        <main className="px-4 lg:px-8 pb-24 lg:pb-8 space-y-5">
          ...
          <div className="rounded-2xl border border-white/8 bg-white/3 overflow-hidden">
            ...
            <div className="p-5 lg:p-7">
              ...
              {ayahError && <p className="text-xs text-red-300/80 mb-3">{ayahError}</p>}
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="rounded-xl border border-white/8 bg-white/2 p-4">
                    ...
                    {tafsirLoading
                      ...
                      : <p className="text-sm leading-7 text-[var(--muted)]">{tafsir?.text ?? "No tafsir available."}</p>
                    }
                  </div>
                  {/* Word by word */}
                  ...
                  {commentsOpen[post.id] && (
                    <div className="rounded-xl border border-white/8 bg-white/2 p-3 space-y-3 mt-1">
                      ...
                      <div className="flex gap-2 pt-1">
                        <input className="flex-1 rounded-xl border border-white/10 bg-white/4 px-3 py-2 text-sm placeholder:text-[var(--muted)] focus:border-white/20 focus:outline-none"
      ...
      {/* ── Right panel (members) — desktop fixed, mobile sheet ── */}
      <aside className={`
        xl:fixed xl:right-0 xl:top-0 xl:h-full xl:w-80 xl:border-l xl:border-white/6 xl:bg-[rgba(15,14,12,0.8)] xl:backdrop-blur-xl xl:z-20 xl:flex xl:flex-col xl:p-5 xl:gap-4 xl:block
      ...
      `}>
        ...
        <div>
          ...
          <p className="text-sm text-[var(--muted)] mb-4">{reflectedCount} of {room?.member_count ?? members.length} reflected today</p>
          <div className="space-y-2">
            {members.map((m) => (
              <div key={m.user_id} className="flex items-center justify-between rounded-xl border border-white/8 bg-white/3 px-3 py-2.5">
                <div className="flex items-center gap-2.5">
                  <Avatar name={m.username} active={m.has_reflected_today} size="sm" />
                  <div>
                    <p className="text-sm">{m.username}</p>
                    <p className="text-xs text-[var(--muted)]">{m.has_reflected_today ? "Reflected ✓" : "Waiting…"}</p>
                  </div>
                </div>
      ...
      <Sheet open={inviteOpen} title="Invite to your circle" kicker="Invite" onClose={() => setInviteOpen(false)}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button type="button" className="rounded-xl border border-white/10 bg-white/4 py-2.5 text-sm" onClick={() => room?.invite_code && navigator.clipboard.writeText(room.invite_code)}>Copy Code</button>
          <button type="button" className="rounded-xl bg-[var(--gold)] py-2.5 text-sm font-medium text-[var(--ink)]" onClick={() => room?.invite_code && navigator.share?.({ text: room.invite_code })}>Share</button>
        </div>
      </Sheet>
      ...
      <Sheet open={collectionSheetOpen} title="Add this ayah" kicker="Collections" onClose={() => setCollectionSheetOpen(false)}>
        ...
        <input className="w-full rounded-xl border border-white/10 bg-white/4 px-4 py-2.5 text-sm placeholder:text-[var(--muted)] focus:outline-none" placeholder="Or create new collection…" value={newCollectionName} onChange={(e) => setNewCollectionName(e.target.value)} />
      ...
      <Sheet open={noteSheetOpen} title="Write a private note" kicker="Note" onClose={() => setNoteSheetOpen(false)}>
        <textarea className="w-full rounded-xl border border-white/10 bg-white/4 px-4 py-3 text-sm placeholder:text-[var(--muted)] focus:outline-none min-h-[120px] resize-none" placeholder="Your private reflection on this ayah…" value={noteBody} onChange={(e) => setNoteBody(e.target.value)} />
